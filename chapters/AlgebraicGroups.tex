...
\begin{itemize}
    ...
    fixed LEX on $\bbF[X_1,\dots, X_n]$, we can define the leading term of a polynomial: 
...
\begin{notation}
    Let $M$ be a $A-$module (where $A$ is a $k$-algebra), then we define
    \begin{equation}
        \text{Der}_k(A,M)=\{D\in\Hom_k(A,M)\;\big|\;D(xy)=x\cdot D(y) + y\cdot D(x),\;\forall x,y\in A\}.
    \end{equation} 
    Observe that since $M$ is a $A-$module, it's also a $k-$vector space with action given by 
    $\alpha\cdot m\equiv (\alpha 1_A)\cdot m,\alpha\in k$.
\end{notation}

  \begin{notation}
    Let $A=\bbF[X_1,\dots,X_n]/I$, where $M$ is a $A-$module. We write 
    \begin{equation}
        \mathcal{T}_{A,M}=\{(v_1,\dots,v_n)\in M^n:\sum_i[D_i(f)]\cdot v_i=0,\;\forall f\in I\}.
    \end{equation}
  ...
  \begin{definition}
    ...
    \begin{equation}
        d\varphi_p: \text{Der}_{\bbF}(A[V],k_p)\to\text{Der}_{\bbF}(A[W],k_q):D_v\mapsto D_v\circ\varphi^*
    \end{equation}
  \end{definition}
