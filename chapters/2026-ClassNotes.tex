...
\begin{remark}
    ...
    $X\cap D(x_i) \times Y\cap D(y_j)\subset D(x_i)\times D(y_j)$ closed ?. We have 
...
\begin{equation}
    \begin{split}
&S_{d,d+1} = \Gr_1(V)\times\cdots\times\Gr_{d-1}(V)\times\{(F,F^\prime)\in\Gr_d(V)\times \Gr_{d+1}(V):F\subset F^\prime\}\times\cdots\\
&\quad\quad\quad\quad\cdots\times\Gr_{d+2}(V)\times\cdots\times \Gr_n(V) 
    \end{split}
\end{equation}
is closed. Therefore 
\begin{equation}
    \mathcal{F}(V) = \bigcap_{d=1}^{n-1}S_{d,d+1}
\end{equation}
is closed, so it's a projective variety.