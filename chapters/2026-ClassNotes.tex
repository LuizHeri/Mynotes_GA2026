...
see that $a_{pq}=\pm\det(a_{ij_k})_{j,k=1}^d=\pm X_{J_{pq}}(w_1\wedge\cdots\wedge w_d)$. So, for any 
$J=(j_1,\dots,j_d)\in\mathcal{J}$, the expression $\det(a_{ij_k})_{i,k=1}^d$ is a polynomial 
$f_J(X)\in k[X_{J_{pq}}: 1\leq p\leq d, d<q\leq n]\subset k[\bigwedge^d V]$ and we see that 
\begin{equation}
    \begin{split}
        &\varphi_{(1,\dots,d)}(\text{Im}(\psi)\cap D(X_{(1,\dots,d)}))\\
        &\quad =V(X_J-f_J(X):\forall J=(j_1,\dots,j_d)\in\mathcal{J}\setminus\{(1,\dots,d)\})
    \end{split}
\end{equation}
